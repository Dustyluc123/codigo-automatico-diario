...
Tarefas a fazer:
- [ ] Adicionar um botão "Adicionar ao Carrinho" a cada `product-card` no arquivo `js/main.js`. O botão deve ser funcional, exibindo um `console.log` ao ser clicado, indicando qual produto foi "adicionado" (usando o `product.name` ou `product.id`). Isso envolverá a modificação da função `renderProducts` para criar o botão e anexar um `event listener`.
