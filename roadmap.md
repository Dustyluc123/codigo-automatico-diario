...
Tarefas a fazer:
- [ ] Criar os arquivos `css/style.css` e `js/main.js` com conteúdo básico para testar a configuração do Vite e garantir que os links no `index.html` estejam funcionando corretamente.
  Detalhes:
  - Para `css/style.css`: Adicionar um estilo simples, como um `background-color` para o `body`.
