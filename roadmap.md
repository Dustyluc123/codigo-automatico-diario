...
Tarefas a fazer:
- [ ] Criar um novo módulo JavaScript (`js/cart.js`) para gerenciar a lógica do carrinho de compras. Este módulo deve exportar funções como `addToCart(product)`, `getCartItems()`, e `removeFromCart(productId)`. Inicialmente, o carrinho pode ser um array vazio armazenado em memória.
