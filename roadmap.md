...
Tarefas Feitas:
- [X] Crie o arquivo HTML principal que servirá como o ponto de entrada para o aplicativo E-Commerce.
  Detalhes:
...
- [X] Adicionar um elemento HTML para exibir o número total de itens no carrinho. Este elemento, uma `div` com o ID `cart-counter`, foi colocado dentro do `header` de `index.html`. Estilização do header foi adicionada para conter esse elemento e o título do e-commerce.
  Detalhes:
  - Modificação do arquivo `index.html` para incluir `<div id="cart-counter">0</div>` dentro do `header`.
  - Adição de estilos `<style>` inline em `index.html` para o `header` e `#cart-counter` para posicionamento e aparência, garantindo que o contador seja exibido visualmente.
- [X] Exibir o número total de itens no carrinho em um indicador visual na interface:
    - [X] No `js/main.js`, criar uma função `updateCartCounter()` que use `getCartItems()` para obter o número de itens e atualize o texto desse elemento.
    - [X] Chamar `updateCartCounter()` após cada `addToCart()` e também na inicialização da página.
  Detalhes:
  - Criação da função `updateCartCounter()` em `js/main.js` que obtém o elemento `#cart-counter`, utiliza `getCartItems()` para contar os itens e atualiza o `textContent` do contador.
  - Integração da chamada a `updateCartCounter()` no `event listener` do botão "Adicionar ao Carrinho" (após a adição de um item).
  - Adição de uma chamada a `updateCartCounter()` na inicialização da página (`js/main.js`) para garantir que o contador reflita o estado inicial do carrinho.
...
<head>
    ...
    <style>
        /* Estilos para o header e o contador de carrinho */
        header {
            display: flex;
            justify-content: space-between; /* Alinha o título e o contador nas extremidades */
        ...
        #cart-counter {
            background-color: #007bff; /* Cor de fundo para o contador */
            color: white;
            padding: 8px 12px;
            border-radius: 20px; /* Bordas arredondadas para um visual de 'badge' */
            font-weight: bold;
            font-size: 1em;
            min-width: 30px; /* Garante um tamanho mínimo para o badge */
            text-align: center;
            box-shadow: 0 2px 5px rgba(0, 123, 255, 0.3);
        }
        /* Adicionalmente, você pode querer reintroduzir a navegação mais tarde */
        /* .header-nav {
            display: flex;
            align-items: center;
        } */
...
<body>
    ...
    <header>
        <!-- O h1 e o contador estão agora diretamente no header para facilitar o flexbox -->
        <h1>Meu E-Commerce</h1>
        <div id="cart-counter">0</div>
        <!-- A navegação pode ser adicionada aqui posteriormente, talvez dentro de um novo elemento -->
        <!-- <nav>
            <ul>
